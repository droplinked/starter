...
<Steps>
  ...
  <Step title="Agent fetches the tool list">
    ...
    ```
    Returns a shop-bound subset of the platform roster — `list_products`, `search_products`,
    `get_product_attestation`, `get_merchant_info` — with no shop-slug argument required on
    every call. The full roster lives on the platform-wide
    [MCP server](/agentic/mcp-server); read it from `tools/list` rather than assuming the two
    surfaces match.
  </Step>
  <Step title="Agent invokes tools">
    ```bash
    curl -X POST https://mcp.droplinked.com/your-shop-slug/mcp/tools/get_merchant_info \
      -H 'content-type: application/json' \
      -d '{}'
    ```
    The scoped surface is a REST shim: a plain `POST` with the tool arguments as the body — no
    JSON-RPC envelope. From here the agent runs the usual MCP loop; for catalogue search across
    every shop, call `search_products` on the platform-wide
...
- **The [ACP product feed](/agentic/acp-feed)** — every Droplinked shop's catalog. Agentic
  shopping surfaces ingest the delimited artifact at
  `https://droplinked-public-feeds.s3.amazonaws.com/acp/latest.tsv.gz`; the JSON endpoint at
  `https://apiv3.droplinked.com/feed/acp.json` is for direct queries, not ACP ingestion (see
