...
The standard [ACP product feed](/agentic/acp-feed) is open and free — every catalog item
Droplinked exposes to agentic shoppers lives there. The **premium** endpoint at
`/feed/acp.premium.json` returns a richer, ranked, agent-tuned slice of the same inventory
(higher refresh cadence, attribution-weighted ordering, more granular merchant signals) — and it
is **gated by the x402 HTTP 402 micropayment protocol**.

<Warning>
  Like `/feed/acp.json`, this is a **JSON API, not an ACP-ingestible file**. The ACP file spec
  accepts tab- or comma-delimited `.tsv`, `.txt`, or `.csv` and rejects JSON — so an ACP
  ingester cannot read the premium endpoint either. Use it from your own code, and submit the
  delimited artifact
  (`https://droplinked-public-feeds.s3.amazonaws.com/acp/latest.tsv.gz`) to ingesters. There is
  no x402-gated delimited artifact today. Background:
...
1. Agent: GET /feed/acp.premium.json                       (no payment)
   ...
   Body: {
     ...
     "extra":    { "name": "USD Coin", "version": "2", "currency": "USDC" }
   }
   (the same challenge is mirrored base64-encoded in the `X-Payment-Required` header)
...
`extra.name` and `extra.version` are the **EIP-712 domain** the transfer authorisation must be
signed over. Sign against the domain the challenge gives you (`name: "USD Coin"`,
`version: "2"`) — a signature produced over any other domain will not verify, and a plain USDC
transfer that skips the authorisation entirely is not a payment proof.

Fetch the live challenge:

```bash
curl -sS https://apiv3.droplinked.com/feed/acp.premium.json | jq .
```
...
consume the open JSON endpoint can swap endpoints. The differences:
...
broader x402 ecosystem — Storyflo's premium-briefing MCP tool uses the same facilitator and
settlement pattern as a reference integration.

---

## Related

- [ACP product feed](/agentic/acp-feed) — the open, unauthenticated version, including the
  delimited artifacts an ACP ingester can actually read
