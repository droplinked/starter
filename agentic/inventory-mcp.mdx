...
| `findInventory` | SKU-availability-aware catalog query — returns `stock_state`, `source`, `priceCents`, `currency`, `imageUrl`, `productUrl`, and a (policy-redacted) attestation status. Designed for cart composition. See [below](#about-findinventory). |
...
`findInventory` is the cart-composition primitive — it queries the catalog and returns each
match with stock state, price, image, and a buyable product URL, so an agent can build a cart
across results.

Today it queries the **native Droplinked catalog** (published, visible, purchasable
`ProductV2` / `ProductSkuV2`). The `source` argument accepts `native`, `impact_brand`,
`rakuten`, or `any` for forward compatibility, but the `impact_brand` and `rakuten` branches
are **stubs** that currently return no rows — the multi-brand affiliate catalogs are reached
today through the merchant-scoped [Impact connector](#multi-brand-impact-catalog), not through
`findInventory`. Likewise the per-item `attestation` field is a privacy-redacted **stub**
(`unattested` by default) until the EAS Schema C inventory reader is wired in; the redaction
respects each merchant's attestation policy, so it never over-discloses.

When `DISCOVERY_RANKER_ENABLED=true`, native results are re-ordered by their merchant's
...
The large multi-brand affiliate catalog (Flatlay × Impact.com Partner REST) is exposed through
a **separate, merchant-authenticated** connector — not the public MCP tools. It is read-only,
query-time (never bulk-replicated), cached ~5 min per query, and rate-limited to 1000/hr per
merchant.
...
All four require a merchant JWT (PRODUCER role). The connector **never** exposes write methods,
even when the token carries RW scope.
