...
This page walks through the per-platform webhook setup for the four most-requested origin platforms beyond Shopify. The droplinked side is identical in each case: one normalized catalog mirror, one per-merchant MCP surface, one [ACP product feed](/agentic/acp-feed) entry, one lender-routing eligibility.
...
- **ACP feed inclusion** — your catalog lands in the delimited artifact an ACP ingester reads,
  `https://droplinked-public-feeds.s3.amazonaws.com/acp/latest.tsv.gz`, and in the JSON
  catalogue API at `apiv3.droplinked.com/feed/acp.json` for direct queries. The two are not
  interchangeable — see [Feed formats](/agentic/overview#feed-formats)
