...
**Privacy**: operator-private fields (`decidedBy`, `decisionReason`, `mintError`, `mintAttempts`, `merchantId`, free-text `notes`) are **scrubbed** from this response. They only surface on the SUPER_ADMIN admin route.
