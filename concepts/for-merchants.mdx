...
flowchart TD
    ...
    B --> C["ACP product feed<br/>(delimited artifact for ingesters<br/>+ JSON API for direct queries)"]
...
<Steps>
  ...
  <Step title="Verify your storefront surfaces">
    ...
    ```bash
    # ACP feed rows for your shop — the delimited artifact is the complete
    # catalogue in one file, and the format an ACP ingester reads
    curl -s https://droplinked-public-feeds.s3.amazonaws.com/acp/latest.tsv.gz \
      | gunzip -c \
      | awk -F'\t' 'NR==1 || $9 ~ /your-shop-slug/' \
      | cut -f1,2,6,13
    ...
    Feed columns are 1-indexed: `9` is `seller_url`, `13` is `is_eligible_checkout`. The JSON
    endpoint at `apiv3.droplinked.com/feed/acp.json` is for direct queries only — it is
    cursor-paginated, narrower than the delimited artifact, and not ACP-ingestible. See
  ...
  <Step title="Request your brand attestation">
...
| ACP product feed | `droplinked-public-feeds.s3.amazonaws.com/acp/latest.tsv.gz` (ingestible) · `apiv3.droplinked.com/feed/acp.json` (JSON API) | Catalog-wide discovery for shopping agents |
